...
Generates realistic change request CSV data for ITSM analysis
"""
...
def generate_change_data(count=15):
    """Generate realistic change request data"""
    ...
    # Blackout windows (from rules document)
    blackout_windows = [
    ...
    for i in range(count):
        ...
        # Approvals (sometimes missing)
        all_approvals = ["Manager", "CAB", "Business Owner", "Security Team"]
        if change_type == "Standard":
            ...
            if random.random() < 0.2:  # 20% missing approvals
            ...
            if random.random() < 0.3:  # 30% missing approvals
                obtained_approvals = ["Manager"]  # Missing CAB
            else:
                obtained_approvals = required_approvals

        # Testing evidence (sometimes missing)
        testing_required = risk_level in ["Critical", "High"]
        if testing_required:
            testing_completed = random.random() > 0.25  # 25% don't test
        else:
            testing_completed = random.random() > 0.5

        # Rollback plan (sometimes missing)
        rollback_documented = random.random() > 0.30  # 30% missing

        # Implemented during blackout (sometimes violates)
        ...
        during_blackout = (
            ...
            (implemented.day > 24 and random.random() < 0.3)  # Last week of month
        )

        # Implementation status
        if random.random() < 0.10:  # 10% fail
            impl_status = "Failed"
            success_verified = False
        elif random.random() < 0.05:  # 5% partial
        ...
        pir_completed = random.random() > 0.20  # 20% don't complete PIR

        # KB article updates
        kb_updated = random.random() > 0.35  # 35% don't update KB
...
if __name__ == "__main__":
    print("🔄 Generating Change Request Data...")
    df = generate_change_data(15)
    ...
    print(f"\nPreview:")
    print(df[["Change_ID", "Type", "Category", "Risk_Level", "Implementation_Status"]].head())
    print(f"\n📊 Statistics:")
    print(f"  Normal: {len(df[df['Type']=='Normal'])}")
    print(f"  Standard: {len(df[df['Type']=='Standard'])}")
    print(f"  Emergency: {len(df[df['Type']=='Emergency'])}")
