...
# ITSM Process Deviation Detection Agent - CSV Version (No Excel Dependencies)
# This version uses CSV files instead of Excel to avoid openpyxl dependency issues
...
def create_sample_incidents_csv():
    """Create sample incident data as CSV file"""
    print("Creating sample incident data (CSV format)...")
    ...
    print(f"✅ Created sample_incidents.csv with {len(incidents)} incidents")
...
class ITSMProcessDeviationAgent:
    ...
    def load_data(self, file_path='sample_incidents.csv'):
        """Load incident data from CSV file - creates sample data if not found"""
        try:
            # Check if file exists in current directory
            if not os.path.exists(file_path):
                print(f"File {file_path} not found. Creating sample data...")
                self.data = create_sample_incidents_csv()
    ...
    def export_results(self, filename='deviation_analysis_results.csv'):
        ...
        try:
            ...
            self.deviation_df.to_csv(filename, index=False)
            ...
            # Export summary as separate CSV
            summary_stats.to_csv('deviation_summary.csv', index=False)
            ...
            category_analysis.to_csv('deviation_category_analysis.csv')

            print(f"✅ Results exported to:")
            print(f"   - {filename}")
            print(f"   - deviation_summary.csv")
            print(f"   - deviation_category_analysis.csv")
...
def main():
    ...
    print("🚀 ITSM Process Deviation Detection Agent (CSV Version)")
    print("======================================================")
    print(f"📍 Working directory: {os.getcwd()}")
    print("📝 Note: This version uses CSV files (no Excel dependencies)")
    ...
    if agent.load_data():
        ...
        print("   - sample_incidents.csv (input data)")
        print("   - deviation_analysis_results.csv (detailed results)")
        print("   - deviation_summary.csv (summary statistics)")
        print("   - deviation_category_analysis.csv (category breakdown)")

        # Show available files
        print("\n📁 Files in current directory:")
        for file in os.listdir('.'):
            if file.endswith(('.csv', '.py')):
                print(f"   ✅ {file}")
    else:
